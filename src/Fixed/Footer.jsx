...
export default function Footer() {
  ...
  return (
    ...
    <div className='flex flex-col items-center gap-7 lg:flex-row lg:justify-between '>
        <div className='flex  items-center gap-2'>
                ...
                <div className='flex flex-col gap-4 items-center cursor-pointer lg:flex-row lg:gap-8 lg:items-start '>
                            {menuItems.map((item) =>
                            <a key={item.title} href={item.url} className='hover:bg-primary hover:box-decoration-clone hover:text-secondary hover:px-1 hover:py-1 hover:rounded-md md:text-md'>
                                {item.title}
                            </a>)}
                        </div>
    ...
    <h1 className="font-bold text-xl text-secondary lg:mb-5  ">
        <span className="bg-primary box-decoration-clone px-1 rounded-md md:texlgxl">Contact Us:</span>
      </h1>
    ...
    <p>Moonstone City, Stardust State 12345</p>
        ...
        <div className='bg-faint p-5 rounded-2xl py-8 lg:w-3/5'>
          <form  className='flex flex-col gap-5 lg:flex-row md:text-md lg:justify-between md:px-8 lg:px-2' onSubmit={handleSubmit}>
